...
import { CreatePartner } from ".";

export const PartnersList: FC<TPartnersProps> = ({ data = [], ...props }) => {
  const [isModalOpen, setModalOpen] = useState(false);
  const [currentPartner, setCurrentPartner] = useState<TPartners | null>(null);
        ...
        {data.map((partner) => (
          ...
          >
            ...
            <div className="absolute flex items-center gap-3 top-5 right-5 opacity-0 transition group-hover:opacity-100">
              <Button onClick={(e) => handleAction(e, partner, 'onEdit')} variant="secondary" asChild>
                <Edit />
              </Button>
